...
################################################################################
# Source: https://github.com/alexmrqt/micropython-sgp30
################################################################################
...
class SGP30:
    def __init__(self, i2c, address=_SGP30_DEFAULT_I2C_ADDR):
        ...
        print("featureset", featureset, _SGP30_FEATURESET)
...
if __name__ == "__main__":
    ...
    sgp30 = SGP30(i2c_1, 88)
    scd30 = SCD30(i2c_2, 97)
    print(sgp30._i2c_read_words_from_cmd([0x20, 0x08], 0.01, 1))
    
    #start = time.time_ns()

    while True:
        sgp30_co2eq, sgp30_tvoc = sgp30.iaq_measure()
        print("Read data from SGP30")
        ...
        # co2, temperature, relh
        scd30_co2, scd30_temp, scd30_relh = scd30.read_measurement()
        print("Read data from SCD30")
        data = (sgp30_co2eq, sgp30_tvoc, scd30_co2, scd30_temp, scd30_relh)
        #print("CO2eq = %d ppm, TVOC = %d ppb" % (co2eq, tvoc))
        print(str(data))
        uart.write(str(data) + "\r\n")
        print("Sent via uart")
        # debug: view incoming messages Pico->Pi using "minicom -b 115200 -o -D /dev/serial0"
        led.toggle()
        #time.sleep_ms(200)
        #end = time.time_ns()
